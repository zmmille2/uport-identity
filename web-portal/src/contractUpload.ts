...
import { LoggingConfiguration, ContractFactory } from 'meeteric-js';
import { IWeb3Adapter, Ethereum } from 'meeteric-js';
import { IIdentifier, ISigningNotary, IStorage } from 'meeteric-js';
...
class Program {
    ...
    public async DeployContract(rawContract: string, contractName: string, contractParams: any, account: string, signer: ISigningNotary) {
        const fromAddress = new Ethereum.Models.EthereumAddress(account);
